...
(function () {
  ...
  var backoffMs = 2000;          // exponential backoff base
  var MAX_BACKOFF_MS = 5000;    // cap at 5 s
  ...
  function poissonDelay(meanMs) {
    return Math.max(20, Math.min(1000, -meanMs * Math.log(1 - Math.random())));
  }

  // GOD MODE: minimal reaction delay
  function humanReactionDelay() {
    return 20 + Math.floor(Math.random() * 30); // 20–50 ms
  }
