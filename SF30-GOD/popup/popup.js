...
// GOD MODE: no contact URL needed
const CONTACT_URL = "#";
...
async function refreshLicenseStatusRow() {
  ...
  // GOD MODE: always valid
  setStatus(els.licenseStatusRow, "GOD MODE", "success");
  els.tierDetail.textContent = "Unlimited · Max Performance";
...
async function refreshLicenseStatusUI() {
  ...
  // GOD MODE: always active
  setStatus(els.licenseStatus, "GOD MODE ACTIVE", "success");
}
...
async function handleMasterToggle(checked) {
  // GOD MODE: no license needed, just toggle
  if (checked) {
    await setStore({ [KEYS.USER_KEY]: "GOD-MODE", sg_tier: "god", sg_license_exp: 9999999999 });
...
async function handleVerify() {
  // GOD MODE: no verification needed
  setStatus(els.licenseStatus, "GOD MODE — always active", "success");
  refreshLicenseStatusRow();
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // GOD MODE: hide license input
  els.licenseInput.value = "GOD-MODE";
  els.licenseInput.disabled = true;
  els.licenseInput.style.opacity = "0.5";
