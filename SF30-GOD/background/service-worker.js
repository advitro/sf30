...
async function getValidToken() {
  ...
  return { token: "god", exp: 9999999999 };
}
...
async function verifyLicense(key, deviceId) {
  // GOD MODE: always valid, no server calls
  const nowSec = Math.floor(Date.now() / 1000);
  await setState({
    [K.USER_KEY]: key || "GOD-MODE",
    sg_tier: "god",
    sg_license_exp: 9999999999,
    sg_server_validated_at: nowSec,
    sg_device_limit_reason: null,
    sg_device_cooldown_days: 0
  });
  console.log("[SG SW] GOD MODE — license auto-validated");
  return { ok: true, tier: "god", exp: 9999999999 };
}
...
chrome.runtime.onInstalled.addListener(async (details) => {
  console.log("[SG SW] onInstalled — GOD MODE — reason:", details.reason);
  const legacy = await new Promise(r => chrome.storage.local.get({
    SG_deviceId: "",
    deviceId: ""
  }, r));
  ...
  // GOD MODE: auto-enable and auto-validate on install
  await verifyLicense("GOD-MODE", "god-device");
  await setState({ [K.ENABLED]: true });
  // Broadcast enable to content scripts
  const tabs = await chrome.tabs.query({ url: "https://atoz.amazon.work/*" });
  for (const tab of tabs) {
    chrome.tabs.sendMessage(tab.id, { type: "SG_SET_ENABLED", value: true }).catch(() => {});
  }
  await withAlarmLock(async () => {
    await clearAllAlarms();
    const st = await getState();
  ...
  console.log("[SG SW] GOD MODE — auto-enabled and scheduled");
});
